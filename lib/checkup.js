...
(function (root, factory) {
  ...
  function chipsDimension(sec, token) {
    ...
    rawTop.forEach((h) => {
      ...
      if (a && pair && a === pair) { excluded.push({ address: a, pct: num(h.pct, 0), why: '流动性池储备' }); return; }
      if (BURN[a]) { excluded.push({ address: a, pct: num(h.pct, 0), why: '已销毁' }); return; }
      if (h.isLocked) { excluded.push({ address: a, pct: num(h.pct, 0), why: '已锁仓' }); return; }
      concPct += num(h.pct, 0);
      counted++;
    });
