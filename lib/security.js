...
(function (root, factory) {
  ...
  // ------------------------------------------------------------- RugCheck
  /**
   * 把 RugCheck 市场子对象里出现的地址并进池子集合。
   * 这些子对象在不同 marketType 下有的是字符串、有的是对象，字段名也不统一，
   * 所以只认三种可能落到「地址」语义上的键，认不出就跳过——宁可少收一个地址，
   * 也不能把普通持有人的地址错当成池子剔掉。
   */
  function collectPoolAccounts(v, into) {
    if (!v) return;
    if (typeof v === 'string') { if (v.length >= 20) into[v.toLowerCase()] = 1; return; }
    if (Array.isArray(v)) { v.forEach((x) => collectPoolAccounts(x, into)); return; }
    if (typeof v !== 'object') return;
    for (const k of ['owner', 'address', 'pubkey']) {
      const s = v[k];
      if (typeof s === 'string' && s.length >= 20) into[s.toLowerCase()] = 1;
    }
  }
  ...
  async function rugcheckFull(mint) {
    ...
    // 同一轮里顺手把池子地址收干净，给筹码维度剔除 AMM 储备用。
    const markets = Array.isArray(d.markets) ? d.markets : [];
    let lpLockedPctMax = 0, lpLockedUsdMax = 0, marketCount = 0;
    const pools = {};
    markets.forEach((m) => {
      if (!m) return;
      // markets[].pubkey 就是 AMM 池账户本身。Solana 上前排地址里的「巨鲸」经常是
      // 池子的代币储备账户（其 owner 正好是池账户），把储备当成个人筹码会把新池一律判死，
      // 所以这里把池地址收出来，交给筹码维度连同 owner 一起匹配。
      if (m.pubkey) pools[String(m.pubkey).toLowerCase()] = 1;
      collectPoolAccounts(m.mintAAccount, pools);
      collectPoolAccounts(m.mintBAccount, pools);
      collectPoolAccounts(m.liquidityAAccount, pools);
      collectPoolAccounts(m.liquidityBAccount, pools);
      const lp = m.lp;
      if (!lp) return;
      marketCount++;
    ...
    const out = {
      ...
      marketCount,
      // 池子地址集合（小写）。只存扁平字符串数组：BONK 这类标的有上千个市场，
      // 存结构体会让长缓存条目膨胀到几百 KB，而下游只需要一个集合做成员判断。
        ...
        if (f.ok && f.found) {
          ...
          if (out.contract && !num(out.contract.holderCount, 0) && num(f.totalHolders, 0) > 0) {
            ...
            out.contract.topHolders = th.map((h) => ({
              ...
              // owner 是代币账户的所有者。Solana 上前排「持有人」经常是池子的代币储备账户，
              // owner 正好等于池子地址；丢掉这个字段，筹码维度就没法把它剔出去。
              owner: h.owner || '',
            }));
            out.contract.holders = {
